        ...
        contractFields.map( field => {
            ...
            try {
                ...
                if(typeof field[fieldName] === "string") {
                    //Only query on fields starting with dot
                    if (field[fieldName][0] == ".") {
                        fieldValue = ValueFilter.parse(field[fieldName], document);
                    }
                    //Fields that don't start with dot are copied literally
                    else {
                ...
                // If multiple field values, select first field with a value (iterate backwards and keep last non-null value)
                    ...
                    for(let i=field[fieldName].length-1; i>=0; i--) {
                        ...
                        // TODO: works for EC but breaks HC
                        // if(typeof indexValue[0] === "undefined") indexValue[0] = null;
                        // fieldValue = indexValue[0] ? indexValue[0] : fieldValue;

                        // TODO: Modified for HC, breaks EC
                        if (!indexValue) {
                            fieldValue = null;
                        }
                        ...
                        else if(typeof indexValue[0] === "undefined") {
                            // Do nothing, keep old fieldValue
                            // fieldValue = null;
                        }
                        else {
                            fieldValue = indexValue[0] ? indexValue[0] : fieldValue;
                ...
                console.error("Error: getContractFields:",field,e);
                process.exit(2);
        ...
        doc.agg.aggregations.parties.buckets.map(b => {
            if (b.key.party == doc.party) {
                ...
                this.flags.partyFields.partyExtra.map(extraQuery => {
                    ...
                    let source = b.partyExtra.hits.hits[0]._source;
                    source.description.parties.map( p => {
                        if(typeof p.name === "number") {p.name = p.name.toString();}
                    } )
                    ...
                    // console.log(ValueFilter.parse(filteredExtraQuery,b.partyExtra.hits.hits[0]._source));
                    partyExtra[extraQuery] = ValueFilter.parse(filteredExtraQuery, source);
                })
            }
        })
